...
Features of an attribute are nothing but characteristics that decsribe the attribute in sufficient detail for it to be compared to other attributes and provide some idea of the similarity or dissimilarity between the compared attributes. These characteristics could either be based on the kind of data that attribute holds or based on schema information and specifications. Based on kind of data, these ``discriminants" can be data type, domain and range of data contained by the attribute, length of used space etc. and based on schema specifications, they can contain information about whether is attribute is a key or not and so on. Some of these features are binary, with values as either 0 or 1 while the others lie in the range [0,1]. We have adaptedd from and extended the feature set provided by \cite{ref1} to include a total of 20 ``discriminators". These discriminators are such that they allow us to 
\begin{itemize}
\item Features based on Schema Specification
\begin{enumerate}
\item Data Type
\item Key constraint
\item Unique constraint
\item Not Null constraint
\end{enumerate}
\item Features based on Data Fields
\begin{enumerate}
\item Features for Numeric Data
\item Features for Character Data
\item Features common to both
\end{enumerate}
\end{itemize}
